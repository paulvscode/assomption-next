...
import { newsSchoolQuery } from "@/sanity/lib/queries";
...
export default async function SchoolNewsPage({
  ...
  const news: NewsItem[] = await client.fetch(newsSchoolQuery);

  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <h1 className="mb-8 text-3xl font-bold text-gray-900">{t("schoolNews")}</h1>
      {news?.length > 0 ? (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {news.map((item) => (
            <Link
              key={item._id}
              href={`/${locale}/actualites/${item._id}`}
